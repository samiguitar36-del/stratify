...
export function AnalysisContextForm({ value, onChange }: AnalysisContextFormProps) {
  return (
    <div className="space-y-4 rounded-[28px] border border-ink/10 bg-[linear-gradient(180deg,rgba(16,36,62,0.04),rgba(13,148,136,0.03))] p-5">
      <div>
        <h4 className="font-display text-xl text-ink">Informacion del cliente / analisis</h4>
        <p className="mt-1 text-sm text-slate-600">
          Define el contexto de negocio o asesoria para que cada escenario tenga una lectura lista
          para presentar y defender.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(event) => updateField(value, onChange, "clientName", event.target.value)}
        ...
        <label className="flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(event) => updateField(value, onChange, "projectName", event.target.value)}
        ...
        <label className="flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(event) => updateField(value, onChange, "advisorName", event.target.value)}
        ...
        <label className="flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(event) => updateField(value, onChange, "analysisDate", event.target.value)}
            className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-ink outline-none transition focus:border-ink"
          />
        </label>
      </div>
      ...
      <label className="flex flex-col gap-2">
        ...
        <textarea
          ...
          onChange={(event) => updateField(value, onChange, "executiveNotes", event.target.value)}
          placeholder="Resumen consultivo, supuestos, riesgos y puntos que deben cuidarse."
          rows={4}
          className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-ink outline-none transition focus:border-ink"
        />
      </label>
      ...
      <label className="flex flex-col gap-2">
        ...
        <textarea
          ...
          placeholder="Decision sugerida, siguiente paso o postura final para el cliente."
          rows={3}
          className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-ink outline-none transition focus:border-ink"
        />
      </label>
      ...
      <label className="flex flex-col gap-2">
        ...
        <p className="text-xs text-slate-500">
          Usa texto separado por comas para clasificar el analisis y recuperar escenarios rapido.
        </p>
      </label>
    </div>
  );
